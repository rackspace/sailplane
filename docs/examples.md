...
This section includes some larger examples which use _multiple_ packages.
...
- Configure [Serverless Framework](https://serverless.com) for use with [StateStorage](state_storage.md).
- Cache `StateStorage` result in [ExpiringValue](expiring_value.md).
- Use [LambdaUtils](lambda_utils.md) to simplify the lambda handler function.
- Do dependency injection with [Injector](injector.md).
- Make HTTPS request with [AwsHttps](aws_https.md). No SigV4 signature required on this use.
- Log status and objects via [Logger](logger.md).
 ...
 */
@Injectable({ dependencies: [StateStorage] })
