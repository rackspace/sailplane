...
describe("ExpiringValue", () => {
  ...
  test("progressive test", async () => {
    ...
    // Initialize
    const sut = new ExpiringValue<string>(() => Promise.resolve(factoryValue), 1000);
  ...
  test("doesn't cache failure", async () => {
    ...
    // Initialize
    const sut = new ExpiringValue<string>(() => factoryResponse, 1000);
    expect(sut["value"]).toBeUndefined();
    expect(sut["expiration"]).toEqual(0);
  ...
  test("does cache failure when option selected", async () => {
    ...
    // Initialize
    const sut = new ExpiringValue<string>(() => factoryResponse, 1000, { cacheError: true });
    expect(sut["value"]).toBeUndefined();
    expect(sut["expiration"]).toEqual(0);
